...
class Table(ABC):
	...
	def __validate_table_number(self, value: int, min_number: int, max_number: int):
		if not min_number <= value <= max_number:
			type_repr = 'Outside table'
			if self.type == 'InsideTable':
				type_repr = 'Inside table'

			raise ValueError(f"{type_repr}'s number must be between {min_number} and {max_number} inclusive!")
